## 1.0.10+1 (fork patch — Manan Apps / TechnoSofts)

* Fix: `FlutterV2rayPlugin.onActivityResult` crashed with
  `NullPointerException: MethodChannel$Result.success() on a null object reference`
  when `pendingResult` was null (e.g. VPN permission result redelivered to a
  recreated Activity/plugin instance after the app was backgrounded during the
  Android 13+ notification permission dialog). Added a null check before
  calling `pendingResult.success(...)`.
