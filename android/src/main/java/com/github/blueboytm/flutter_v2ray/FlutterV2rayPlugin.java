...
public class FlutterV2rayPlugin implements FlutterPlugin, ActivityAware, PluginRegistry.ActivityResultListener {
    ...
    public boolean onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (requestCode == REQUEST_CODE_VPN_PERMISSION) {
            // FIX: pendingResult can be null if onActivityResult fires more than once
            // (e.g. another permission dialog's result arrives right before/after this one,
            // or the plugin re-attaches after an activity recreation). Calling
            // .success() on a null Result crashes with:
            // "Attempt to invoke interface method ... MethodChannel$Result.success ... on a null object reference"
